...
static std::string handle_unknown_tag(const GumboStringPiece * const text)
{
  std::string tagname = "";
  if (text->data == NULL) {
    return tagname;
...
static std::string get_tag_name(const GumboNode * const node)
{
  std::string tagname;
  // work around lack of proper name for document parent
...
static std::string build_doctype(const GumboNode * const node)
{
  std::string results = "";
...
static std::string prettyprint(const GumboNode* const, int lvl, const std::string indent_chars);


// prettyprint children of a parent
// may be invoked recursively

static std::string prettyprint_contents(const GumboNode* node, int lvl, const std::string indent_chars) {
  ...
  const GumboVector* const children = &node->v.element.children;
  ...
  for (unsigned int i = 0; i < children->length; ++i) {
    ...
    if (child->type == GUMBO_NODE_TEXT) {
      ...
      if (pp_okay && (contents.length() == 0)) {
        ...
        contents.append(std::string(static_cast<unsigned long>((lvl - 1) * n), c));
      }
...
static std::string prettyprint(const GumboNode* const node, int lvl, const std::string indent_chars) {

  // special case the document parent
  if (node->type == GUMBO_NODE_DOCUMENT) {
  ...
  for (auto i=0u; i < attribs->length; ++i) {
  ...
  std::string indent_space = std::string(static_cast<unsigned long>((lvl - 1) * n), c);
